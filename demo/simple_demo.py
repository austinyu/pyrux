...
class CameraSettings(pr.Slice):
    ...
    roi: tuple[
        Annotated[int, Ge(0)], Annotated[int, Ge(0)], 
    ...
    @pr.reduce
    def set_bit_depth(piece: CameraSettings, payload: int) -> CameraSettings:
...
class MonoImgSettings(pr.Slice):
    ...
    def react_to_roi(piece: MonoImgSettings, state: tuple[int, int, int, int]) -> MonoImgSettings:
        return piece.update({
...
def react_to_black_change(args: tuple[int, float]) -> None:
    print(f"black block to {int(args[1] * (2 ** args[0] - 1))}")

# @pr.subscribe((CameraSettings.bit_depth, MonoImgSettings.white_level))
def react_to_white_change(args: tuple[int, float]) -> None:
    print(f"white block to {int(args[1] * (2 ** args[0] - 1))}")
